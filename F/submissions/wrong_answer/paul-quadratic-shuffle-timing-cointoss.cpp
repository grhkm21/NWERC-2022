...
int main() {
	...
	vector<point> corners_vec(begin(corners), end(corners));
	shuffle(begin(corners_vec), end(corners_vec), gen);

	clock_t start = clock();
	for (const auto &p: corners_vec) {
		if (double(clock()-start)/CLOCKS_PER_SEC >= 7.0) {
			string res = gen() % 2 ? "possible" : "impossible";
			cout << res << '\n';
			return 0;
		}
